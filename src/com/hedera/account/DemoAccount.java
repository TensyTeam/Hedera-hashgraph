...
import com.hedera.account.AccountGetBalance;
...
//    	doAddClaim = true; -- not implemented ?
		...
		if (create) {
			...
			if (account != null) {
				...
				if (balance) {
					System.out.println("МАНИМАНИМАНИ");
			...
			// new account objects
			HederaAccount account = new HederaAccount(ExampleUtilities.getTxQueryDefaults().payingAccountID.shardNum,
					ExampleUtilities.getTxQueryDefaults().payingAccountID.realmNum,
					ExampleUtilities.getTxQueryDefaults().payingAccountID.accountNum);
			HederaAccount accountXferTo = new HederaAccount();
